(async function () {
  ...
  function prever(h) {
    ...
    // Inversão
    if (ult5.every(n => n === 2)) return { cor: "red", texto: "🔁 5 Pretos seguidos: Apostar Vermelho ⚪", previsao: 1 };
    if (ult5.every(n => n === 1)) return { cor: "black", texto: "🔁 5 Vermelhos seguidos: Apostar Preto ⚪", previsao: 2 };

    // Tendência
    const pretos = count(ult7, 2);
    const vermelhos = count(ult7, 1);
    if (pretos >= 5) return { cor: "red", texto: "📊 Tendência: Preto dominante → Apostar Vermelho ⚪", previsao: 1 };
    if (vermelhos >= 5) return { cor: "black", texto: "📊 Tendência: Vermelho dominante → Apostar Preto ⚪", previsao: 2 };
    ...
    return pretos > vermelhos
      ? { cor: "red", texto: "🤖 Probabilidade sugere: Vermelho + ⚪", previsao: 1 }
      : { cor: "black", texto: "🤖 Probabilidade sugere: Preto + ⚪", previsao: 2 };
  }
